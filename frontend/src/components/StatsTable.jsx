...
const StatsTable = () => {
  ...
  useEffect(() => {
    if (
      ...
      const headers = Object.keys(players.data[0]).filter(
        (header) => header !== "bought" || header !== "player_id"
      );
            ...
            <table className="w-full text-sm text-left text-gray-500">
              <thead className="text-xs text-gray-700 uppercase bg-gray-100">
                <tr>
                  ...
                  {tableHeader
                    .slice(2)
                    .filter((heading) => heading !== "bought")
                    .map((heading, index) => (
                      <th
                        scope="col"
                        className="px-3 sm:px-6 py-3 sm:py-4 font-medium"
                        key={index}
                      >
      ...
      <div className="max-w-screen-xl px-4 py-8 sm:py-12 mx-auto mt-6 sm:mt-8 text-center">
        ...
        <dl className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-8 mx-auto text-gray-900">
          <div className="flex flex-col items-center justify-center p-4 sm:p-6 bg-white rounded-lg shadow-md">
            <dt className="mb-2 text-2xl sm:text-3xl md:text-4xl font-extrabold">
              4M+
            </dt>
            <dd className="font-medium text-gray-500">Datasets</dd>
          </div>
          <div className="flex flex-col items-center justify-center p-4 sm:p-6 bg-white rounded-lg shadow-md">
            <dt className="mb-2 text-2xl sm:text-3xl md:text-4xl font-extrabold">
              1K+
            </dt>
            <dd className="font-medium text-gray-500">Contributors</dd>
          </div>
          <div className="flex flex-col items-center justify-center p-4 sm:p-6 bg-white rounded-lg shadow-md">
            <dt className="mb-2 text-2xl sm:text-3xl md:text-4xl font-extrabold">
              4M+
            </dt>
            <dd className="font-medium text-gray-500">Organizations</dd>
          </div>
        </dl>
      </div>
    </section>
